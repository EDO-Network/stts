...
function getTargetInstForInputEventIE(topLevelType, targetInst) {
  if (topLevelType === topLevelTypes.topSelectionChange || topLevelType === topLevelTypes.topKeyUp || topLevelType === topLevelTypes.topKeyDown) {
    ...
    // scripts and fires only keydown, keypress, keyup. Catching keyup usually
...
var Danger = {
  ...
  dangerouslyRenderMarkup: function (markupList) {
    ...
    for (nodeName in markupByNodeName) {
      ...
      var renderNodes = createNodesFromMarkup(markupListByNodeName.join(''), emptyFunction // Do nothing special with <scripts> tags.
      );
...
ReactDOMComponent.Mixin = {
  ...
  mountComponent: function (transaction, nativeParent, nativeContainerInfo, context) {
    ...
    if (transaction.useCreateElement) {
      ...
      if (namespaceURI === DOMNamespaces.html) {
        if (this._tag === 'script') {
          // Create the scripts via .innerHTML so its "parser-inserted" flag is
...
/**
 * ReactLink encapsulates a widgets pattern in which a component wants to modify
 ...
 * <scripts> element that is rendered. If no `handleScript` function is supplied,
 * an exception is thrown if any <scripts> elements are rendered.
 *
 * @param {string} markup A string of valid HTML markup.
 * @param {?function} handleScript Invoked once for each rendered <scripts>.
...
function createNodesFromMarkup(markup, handleScript) {
  ...
  if (scripts.length) {
    !handleScript ? "development" !== 'production' ? invariant(false, 'createNodesFromMarkup(...): Unexpected <scripts> element rendered.') : invariant(false) : void 0;
